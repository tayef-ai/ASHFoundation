...
class NikahRegistration(models.Model):
    ...
    def save(self, *args, **kwargs):
        groom_img = PilImage.open(self.groom_image)
        output_size = (300, 225)
        groom_img = groom_img.resize(output_size)
        buffer = BytesIO()
        img_format = 'JPEG' if self.groom_image.name.lower().endswith('.jpg') or self.groom_image.name.lower().endswith('.jpeg') else 'PNG'
        groom_img.save(buffer, format=img_format, quality=70)
        self.groom_image.save(self.groom_image.name, File(buffer), save=False)
        bride_img = PilImage.open(self.bride_image)
        output_size = (300, 225)
        bride_img = bride_img.resize(output_size)
        buffer = BytesIO()
        img_format = 'JPEG' if self.bride_image.name.lower().endswith('.jpg') or self.bride_image.name.lower().endswith('.jpeg') else 'PNG'
        bride_img.save(buffer, format=img_format, quality=70)
        self.bride_image.save(self.bride_image.name, File(buffer), save=False)
        super().save(*args, **kwargs)
